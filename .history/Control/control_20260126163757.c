...
int velocity(int velocity_left, int velocity_right)
{
    ...
    if(Encoder_Integral > 320000)  	Encoder_Integral = 320000;        //===积分限幅，输出限幅在8000左右，怎么积分限幅可以这么大？？？？
...
int turn(int velocity_left, int velocity_right)   //转向控制
{
    ...
    if(1 == Flag_Left)	           Turn_Target -= 60;
    else if(1 == Flag_Right)	     Turn_Target += 60;
    else Turn_Target = 0;
...
u16  Linear_Conversion(int moto)
{
    ...
    if (temp < 75) Linear_Moto = 75; //最小脉冲个数值60个，比这更低马达功率的扭矩不够，转不起来
    else Linear_Moto = (u16)temp;
